function sig=OverlapAdd_update(X,yphase,windowLen,ShiftLen)
%Xugang Lu @NICT
%sig=OverlapAdd(X,A);
...
for i=1:FrameNum
    ...
    spec=Spec(:,i);
    sig(start:start+windowLen-1)=sig(start:start+windowLen-1)+real(ifft(spec,windowLen)).*hamming(windowLen);       
end
return